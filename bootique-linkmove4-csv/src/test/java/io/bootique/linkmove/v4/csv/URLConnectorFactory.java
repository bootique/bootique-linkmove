...
class URLConnectorFactory implements IConnectorFactory<StreamConnector> {
    ...
    private final Map<String, String> connectorUrls;

    public URLConnectorFactory(Map<String, String> connectorUrls) {
    ...
    public Optional<? extends StreamConnector> createConnector(String id) throws LmRuntimeException {
        return Optional.ofNullable(connectorUrls.get(id))
                .map(ResourceFactory::new)
                .map(ResourceFactory::getUrl)
                .map(URLConnector::of);
    }
}
